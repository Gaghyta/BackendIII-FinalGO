...
func (s *sqlStore) Read(id int) (domain.Odontologo, error) {
	...
	if err != nil {
		if err == sql.ErrNoRows {
			...
			return domain.Odontologo{}, errors.New("odontólogo no encontrado")
		}
...
func (s *sqlStore) Create(p domain.Odontologo) error {
	query := "INSERT INTO odontologos (apellido_odontologo, nombre_odontologo, matricula) VALUES (?, ?, ?);"
	stmt, err := s.db.Prepare(query)
	if err != nil {
		log.Fatal(err)
...
func (s *sqlStore) Update(o domain.Odontologo) error {
	return errors.New("not implemented yet")
}
